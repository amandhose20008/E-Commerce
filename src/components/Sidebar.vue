...
const userData = stores.getters['auth/getUserInfo']
...
<template>
   <nav class="bg-orange-500 shadow-lg h-screen fixed top-0 left-0 min-w-[250px] py-6 px-6 font-[sans-serif] flex flex-col overflow-auto">
    <div class="flex flex-wrap items-center cursor-pointer">
      <div class="relative">
        <img :src="userData.profilePic" class="w-12 h-12 rounded-full border-white" />
        <span class="h-3 w-3 rounded-full bg-green-600 border-2 border-white block absolute bottom-0 right-0"></span>
      </div>
      <div class="ml-4">
        <p class="text-sm text-gray-300">{{ userData.name }}</p>
        <p class="text-xs text-gray-400 mt-1">{{ userData.email }}</p>
      </div>
    </div>
    <div class="relative bg-[#f8f8f9] px-4 py-3 rounded-md my-8">
      ...
      <input v-model="searchText" class="text-sm text-gray-300 outline-none bg-transparent px-1 max-w-[130px]" placeholder="Search..." />
    </div>
    <ul class="space-y-10 flex-1 mt-4 mb-10">
      <!-- Your list items here -->
      <li>
      <RouterLink to="/dashboard" class="text-gray-300 hover:text-white text-sm flex items-center rounded-md">
    ...
    <li>
      <RouterLink to="/product" class="text-gray-300 hover:text-white text-sm flex items-center rounded-md">
    ...
    <ul>
      <li>
        <button class="text-gray-300 hover:text-white text-sm flex items-center rounded-md" @click="logout">
