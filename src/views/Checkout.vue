        ...
        <div class="lg:col-span-2 max-lg:order-1 p-6 max-w-4xl mx-auto w-full">
          ...
          <form class="lg:mt-12">
              ...
              <div class="flex flex-wrap gap-4 mt-8">
                <button type="button" @click="navigateToProduct" class="min-w-[150px] px-6 py-3.5 text-sm bg-gray-100 text-[#333] rounded-md hover:bg-gray-200">Back</button>
                <button type="button" class="min-w-[150px] px-6 py-3.5 text-sm bg-[#333] text-white rounded-md hover:bg-[#111]">Confirm payment ${{ subTotal }}</button>
              </div>
            </form> 
        </div>
          ...
          <div class="relative h-full">
            <div class="p-8 lg:overflow-auto lg:h-[calc(100vh-60px)] max-lg:mb-8">
              ...
              <div class="space-y-6 mt-10">
                <div v-for="(item, index) in cartData" :key="index" class="grid sm:grid-cols-2 items-start gap-6">
                  <div class="max-w-[190px] px-4 py-6 shrink-0 bg-gray-200 rounded-md">
                    <img :src="item.productImage" class="w-full object-contain" />
                  </div>
                  <div>
                    <h3 class="text-base text-[#333]">{{ item.productName }}</h3>
                    <ul class="text-xs text-[#333] space-y-2 mt-2">
                      <!-- <li class="flex flex-wrap gap-4">Size <span class="ml-auto">{{ item.description }}</span></li> -->
                      <li class="flex flex-wrap gap-4">Quantity <span class="ml-auto">{{ item.quantity }}</span></li>
                      <li class="flex flex-wrap gap-4">Total Price <span class="ml-auto">${{ item.totalPrice }}</span></li>
                    </ul>
                  </div>
                </div>
              </div>
            ...
            <div class="absolute left-0 bottom-0 bg-gray-200 w-full p-4">
              <h4 class="flex flex-wrap gap-4 text-base text-[#333] font-bold">Total <span class="ml-auto">${{ subTotal }}</span></h4>
            </div>
          </div>
        </div>
      </div>
...
const cartData = stores.getters['product/getCartList']
